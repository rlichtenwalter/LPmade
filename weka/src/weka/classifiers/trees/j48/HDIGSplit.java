...
/**
 * Class implementing a HD-IG product split on an attribute.
 *
 * @author Ryan Lichtenwalter (rlichten@cse.nd.edu)
 * @version $Revision: 1.00 $
 */
public class HDIGSplit extends ClassifierSplitModel {
	...
	/** HD-IG product of split.  */
	private double m_HDIG;
	 ...
	 */
	public HDIGSplit( int attIndex, int minNoObj, double sumOfWeights ) {
	...
	/**
	 * Creates a HD-IG product split on the given data. Assumes that none of
	...
	/**
	 * Returns HD-IG product distance for the generated split.
	 */
	public final double HDIG() {
		return m_HDIG;
	}
	...
	private void handleEnumeratedAttribute( Instances trainInstances ) throws Exception {
		...
		if( m_distribution.check( m_minNoObj ) ) {
			...
			HDIGSplitCrit hdigSplitCrit = new HDIGSplitCrit();
			m_HDIG = hdigSplitCrit.splitCritValue( m_distribution, m_sumOfWeights );
		}
	}
		...
		while( next < firstMiss ) {
			if( trainInstances.instance( next - 1 ).value( m_attIndex ) + 1e-5 < trainInstances.instance( next ).value( m_attIndex ) ) {
				...
				if( Utils.grOrEq( m_distribution.perBag( 0 ), minSplit ) && Utils.grOrEq( m_distribution.perBag( 1 ), minSplit ) ) {
					double currentDistance = hdigSplitCrit.splitCritValue( m_distribution, m_sumOfWeights );
					if( Utils.gr( currentDistance, m_HDIG ) ) {
						m_HDIG = currentDistance;
						splitIndex = next - 1;
					}
					m_index++;
				}
