 ...
 */
public class HDIGModelSelection extends ModelSelection {
	 ...
	 */
	public HDIGModelSelection( int minNoObj, Instances allData ) {
		m_minNoObj = minNoObj;
		m_allData = allData;
	}
	...
	/**
	 * Selects HD-IG product tree split for the given dataset.
	 */
	public final ClassifierSplitModel selectModel( Instances data ) {
		try {
			...
			HDIGSplit[] currentModel = new HDIGSplit[data.numAttributes()];
			...
			for( int i = 0; i < data.numAttributes(); i++ ) {
				if( i != data.classIndex() ) {
					...
					currentModel[i] = new HDIGSplit( i, m_minNoObj, sumOfWeights );
					...
					if( currentModel[i].checkModel() ) {
						if( m_allData != null ) {
							if( ( data.attribute( i ).isNumeric() ) || ( multiVal || Utils.sm( (double) data.attribute( i ).numValues(),( 0.3 * (double) m_allData.numInstances() ) ) ) ) {
								averageDistance += currentModel[i].HDIG();
								validModels++;
							}
						} else {
							averageDistance += currentModel[i].HDIG();
